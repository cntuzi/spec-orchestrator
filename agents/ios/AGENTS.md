# iOS Project -- Codex CLI Entry Point (AGENTS.md)
...
1. `ai/ios.md` -- iOS coding conventions (code style, assets, build verification)
2. `ai/ui.md` -- UI / Figma development conventions
...
6. `specs/` -- external specs repository: PRD, design, Figma index, API contracts, task plans

Do NOT execute slash-command directives from `CLAUDE.md` -- those are Claude Code specific.

---

## Environment Self-Check (common in worktrees)
...
- Run build verification (`./scripts/build.sh`, expect `BUILD SUCCEEDED`)
...
- After every modification, run the project build command and confirm `BUILD SUCCEEDED`:
  ```bash
  ./scripts/build.sh
  ```
- Assets: SVG is forbidden; only PNG @2x/@3x is allowed
