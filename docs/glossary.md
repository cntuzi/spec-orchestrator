...
| `capabilities` | List of enabled capabilities for this feature (e.g., `[analytics, i18n]`) |
| `analytics[]` | Tracking events -- requires `capabilities: [analytics]` |
| `i18n_ref` | i18n string reference -- requires `capabilities: [i18n]` |
...
A **platform-specific configuration template** that defines how code should be written. Lives in `agents/{platform}/` within the spec-orchestrator repo.

An Agent is NOT a running process -- it is a static configuration layer that includes:

| Component | Purpose |
|-----------|---------|
| `ai/{platform}.md` | Language and architecture coding conventions |
| `ai/ui.md` | UI framework conventions (SwiftUI, Compose, etc.) |
| `CLAUDE.md` | Claude Code entry point (build commands, project structure) |
| `AGENTS.md` | Codex CLI entry point |
| `.claude/config.yaml` | Project config template with specs_path and platform |

**Relationship to Spec**: Spec defines WHAT to do (requirements, acceptance criteria). Agent defines HOW to do it (coding style, build tools, framework rules). They are complementary -- a Worker needs both.

**Relationship to Worker**: Agent is the template; Worker is the runtime instance. When `spec-drive` dispatches a Worker, the Worker inherits Agent conventions from the platform repo and reads Spec files for task context.

**Deployment**: `agents/sync.sh` deploys Agent configuration (shared `_shared/` + platform-specific) into the target platform repository.
...
**Single authoritative source** for internationalization strings. Grouped by Feature, one key + translations per row. **Only generated when the i18n capability is enabled** in `context.yaml`.

Feature YAML and Tasks only reference this file; they do not inline keys. Features must declare `capabilities: [i18n]` to use i18n fields (`i18n_ref`, `i18n_keys`).
